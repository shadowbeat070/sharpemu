...
public sealed unsafe partial class DirectExecutionBackend
{
	...
	// gregs[23] — the kernel's x86-64 sigcontext slot err(152) trapno(160) oldmask(168)
	// cr2(176) fpstate(184), which glibc and musl both overlay verbatim, so the offset is
	// libc-independent. Inside the FXSAVE image XMM0 begins at byte 160 (32-byte header
	// plus 8 legacy x87/MMX slots of 16 bytes), the same relative position it occupies in
	// the Win64 CONTEXT's FltSave area (Win64ContextXmm0Offset = 256 + 160).
	...
	private static bool TryHandlePosixFault(int signal, nint siginfo, nint ucontext)
	{
		...
		// One bridge serves both recovery paths that touch XMM state: Intel SHA
		// emulation and SSE4a EXTRQ/INSERTQ. Both arrive as SIGILL, so gating on
		// it costs neither anything, and GetPosixXmmBase covers Darwin as well as
		// the Linux fpstate pointer. Writes reach the guest through sigreturn.
